...
export function Icon({ name, className = "w-5 h-5" }) {
  return (
    <span
      aria-hidden="true"
      className={`${className} inline-flex items-center justify-center rounded-md bg-slate-700 text-white font-black leading-none select-none shadow-sm`}
    >
      {ICONS[name] || "•"}
    </span>
  );
    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="h-9 w-full rounded-xl bg-slate-950 border border-slate-600 px-3 text-white placeholder:text-slate-400"
        />
...
export function CurrencySelect({ model, value, onChange, includeAll = false }) {
  ...
  return (
    <select
      ...
      className="h-9 w-full rounded-xl bg-slate-950 border border-slate-600 px-3 text-sm text-white"
    >
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          type="button"
          className="h-7 rounded-lg bg-slate-700 hover:bg-slate-600 text-white px-3 text-sm"
          onClick={addEntry}
        >
          {addLabel}
        </button>
      ...
      {safeEntries.length === 0 && (
        <p className="text-xs text-slate-300 rounded-xl bg-slate-950 border border-slate-700 p-2">
          {emptyText}
        </p>
      )}
      ...
      {safeEntries.map((entry, index) => (
        <div
          key={`${entry.currencyId}_${index}`}
          className="grid grid-cols-12 gap-2 items-end rounded-xl bg-slate-950 border border-slate-700 p-2"
        >
          ...
          <div className="col-span-5">
            ...
            <input
              ...
              className="h-9 w-full rounded-xl bg-slate-950 border border-slate-600 px-3 text-white"
            />
          </div>

          <button
            type="button"
            className="col-span-2 h-9 rounded-xl border border-red-400 text-red-100 hover:bg-red-900"
