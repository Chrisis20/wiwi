...
function normalizeArea(area, fallbackCurrencyId) {
  ...
  return {
    id: base.id || "area_1",
    name: base.name || "Area",
    costs: costs.map((cost) => ({ currencyId: cost.currencyId || fallbackCurrencyId, amount: positive(cost.amount, 0) })),
    rewards: rewards.map((reward) => ({ currencyId: reward.currencyId || fallbackCurrencyId, amount: positive(reward.amount, 0) }))
  };
}

export function normalizeModel(model) {
  const source = model || EMPTY_MODEL;
  const currencies = Array.isArray(source.currencies) && source.currencies.length ? source.currencies : clone(EMPTY_MODEL.currencies);
  ...
  return {
    ...
    areas: areas.map((area, index) => normalizeArea({ ...area, id: area.id || `area_${index + 1}`, name: area.name || `Area ${index + 1}` }, fallbackCurrencyId))
  };
}
...
export function formatCurrencyList(model, entries, emptyText = "None") {
  ...
  return entries.map((entry) => `${compact(entry.amount)} ${currencyName(model, entry.currencyId)}`).join(" + ");
}

export function getMultiplier(model, levels, currencyId) {
  return model.upgrades.reduce((total, upgrade) => {
    if (upgrade.affectsCurrencyId !== currencyId && upgrade.affectsCurrencyId !== "all") return total;
...
export function getIncomePerSecond(model, levels, areaIndex) {
  ...
  model.generators.forEach((gen) => {
    ...
    income[gen.currencyId] = (income[gen.currencyId] || 0) + (gen.baseAmount / gen.everySeconds) * areaBoost * multiplier;
  });
  return income;
}
...
export function simulateEconomy(rawModel) {
  ...
  for (let step = 0; step <= secondsTotal * ticksPerSecond; step += 1) {
    ...
    const availableUpgrades = model.upgrades
      ...
      .map((upgrade) => {
        ...
        const affectedCurrencyIds = upgrade.affectsCurrencyId === "all" ? model.currencies.map((currency) => currency.id) : [upgrade.affectsCurrencyId];
        ...
        const incomeGain = affectedCurrencyIds.reduce((sum, currencyId) => sum + Math.max((afterIncome[currencyId] || 0) - (beforeIncome[currencyId] || 0), 0), 0);
        const roiSeconds = cost / Math.max(incomeGain, 0.000001);
        return { ...upgrade, cost, roiSeconds };
      })
    ...
    for (const upgrade of availableUpgrades) {
      if ((balances[upgrade.currencyId] || 0) + 1e-9 >= upgrade.cost && upgrade.roiSeconds <= model.targetTimeSeconds * 2.5) {
        ...
        purchaseLog.push({ time: t, name: upgrade.name, level: levels[upgrade.id], cost: upgrade.cost, currencyId: upgrade.currencyId });
        break;
      }
    }
    ...
    if (t - lastTimelineSecond >= 60 || step === 0 || step === secondsTotal * ticksPerSecond) {
      ...
      const row = { minute: Math.round(t / 60), area: currentAreaIndex + 1 };
  ...
  const areaRows = model.areas.map((area, index) => {
    ...
    return { ...area, index: index + 1, unlockTime, segmentTime, unlocked: unlockTime != null };
  });

  return { model, balances, levels, finalIncome, areaRows, timeline, purchaseLog, currentAreaIndex };
}
...
export function getWarnings(rawModel, sim) {
  ...
  if (model.generators.length === 0 && model.areas.length > 1) warnings.push({ type: "bad", text: "You have unlockable areas but no income sources." });

  sim.areaRows.forEach((area, index) => {
    if (index === 0) return;
    if (!area.unlocked) {
      warnings.push({ type: "bad", text: `${area.name} was not reached. A required currency may be too expensive or impossible to earn.` });
      return;
    }
    if (area.segmentTime > target * 2) warnings.push({ type: "bad", text: `${area.name} takes too long to reach: ${timeText(area.segmentTime)} after the previous area.` });
    if (area.segmentTime < target * 0.25) warnings.push({ type: "warn", text: `${area.name} is reached too fast: ${timeText(area.segmentTime)} after the previous area.` });
    const previous = sim.areaRows[index - 1];
    if (previous?.segmentTime && area.segmentTime / previous.segmentTime > 2.5) warnings.push({ type: "bad", text: `Difficulty spike before ${area.name}: ${Math.round(area.segmentTime / previous.segmentTime)}x slower than the previous step.` });
  });

  model.currencies.forEach((currency) => {
    const hasGenerator = model.generators.some((gen) => gen.currencyId === currency.id && gen.baseAmount > 0);
    const hasAreaReward = model.areas.some((area) => area.rewards.some((reward) => reward.currencyId === currency.id && reward.amount > 0));
    const isSpent = model.upgrades.some((upgrade) => upgrade.currencyId === currency.id) || model.areas.some((area) => area.costs.some((cost) => cost.currencyId === currency.id && cost.amount > 0));
    if (isSpent && !hasGenerator && !hasAreaReward && currency.startingAmount <= 0) warnings.push({ type: "bad", text: `${currency.name} is used as a cost but has no source.` });
  });

  model.upgrades.forEach((upgrade) => {
    if (upgrade.multiplierPerBuy <= 1) warnings.push({ type: "warn", text: `${upgrade.name} has no real multiplier. Set multiplier per buy above 1.` });
    if (upgrade.costGrowth > upgrade.multiplierPerBuy * 2.2) warnings.push({ type: "warn", text: `${upgrade.name} cost grows much faster than its multiplier. It may stop being worth buying early.` });
  });

  if (warnings.length === 0) warnings.push({ type: "good", text: "No major economy problems detected in this simulation." });
...
export function suggestNextArea(rawModel, sim) {
  ...
  return { name: `Area ${model.areas.length + 1}`, costs: [{ currencyId: spendCurrency, amount: Math.max(1, income * model.targetTimeSeconds) }], rewards: [] };
}
...
export function runTests() {
  ...
  tests.push({ name: "passive Rune income uses amount / seconds", pass: Math.abs(income.rune - 5) < 0.0001 });
  tests.push({ name: "passive Essence income uses amount / seconds", pass: Math.abs(income.essence - 0.2) < 0.0001 });
  tests.push({ name: "multi-cost areas are supported", pass: model.areas[2].costs.length === 2 });
  tests.push({ name: "multi-reward areas are supported", pass: model.areas[2].rewards.length === 2 });
  tests.push({ name: "canAffordCosts requires every currency", pass: canAffordCosts({ rune: 100, essence: 5 }, [{ currencyId: "rune", amount: 50 }, { currencyId: "essence", amount: 6 }]) === false });
  tests.push({ name: "payCosts subtracts multiple currencies", pass: (() => { const b = { rune: 100, essence: 10 }; payCosts(b, [{ currencyId: "rune", amount: 40 }, { currencyId: "essence", amount: 3 }]); return b.rune === 60 && b.essence === 7; })() });
  tests.push({ name: "giveRewards adds multiple currencies", pass: (() => { const b = { rune: 0, soul: 0 }; giveRewards(b, [{ currencyId: "rune", amount: 5 }, { currencyId: "soul", amount: 2 }]); return b.rune === 5 && b.soul === 2; })() });
  const sim = simulateEconomy(model);
  tests.push({ name: "simulation creates timeline", pass: sim.timeline.length > 1 });
  tests.push({ name: "simulation returns finite income", pass: Object.values(sim.finalIncome).every(Number.isFinite) });
  tests.push({ name: "suggested next area has costs array", pass: Array.isArray(suggestNextArea(model, sim).costs) });
  tests.push({ name: "compact formats thousands", pass: compact(1500) === "1.50K" });
  return tests;
}
